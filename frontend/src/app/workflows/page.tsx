...
export default function WorkflowsPage() {
  ...
  const filtered = workflows.filter(w => {
    ...
    const okSearch = !searchQuery || w.action_name?.toLowerCase().includes(searchQuery.toLowerCase()) || w.id?.toLowerCase().includes(searchQuery.toLowerCase()) || w.trace_id?.toLowerCase().includes(searchQuery.toLowerCase())
      ...
      <header className="border-b bg-white/80 sticky top-0 z-40">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-xl font-bold">Workflows</h1><p className="text-sm text-slate-500">{workflows.length} executions</p></div>
          </div>
